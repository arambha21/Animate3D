...
class MVUNetMotionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def __init__(
        ...
        # MVDream
        camera_embedding_dim: Optional[int] = None,
    ):
        super().__init__()
        ...
        # MVDream: camera embedding
        if camera_embedding_dim is not None:
    ...
    def from_unet2d(
        ...
        # MVDream
        model.camera_embedding.load_state_dict(unet.camera_embedding.state_dict())
    ...
    def forward(
        ...
        r"""
        The [`MVUNetMotionModel`] forward method.
        ...
        """
        # MVDream: sample check
        assert sample.shape[0] % num_views == 0, "[UNet] input batch size must be dividable by num_views!"
        ...
        if i2v_cond_time_zero:
            cond_timesteps = torch.zeros(sample.shape[0]).to(timesteps)
            cond_t_emb = self.time_proj(cond_timesteps)

            cond_t_emb = cond_t_emb.to(dtype=self.dtype)
            cond_emb = self.time_embedding(cond_t_emb, timestep_cond)

        # MVDream: Add camera embeddings
        if camera is not None:
            assert camera.shape[0] == emb.shape[0]
            camera_emb = self.camera_embedding(camera)
            emb = emb + camera_emb
            if i2v_cond_time_zero:
                cond_emb = cond_emb + camera_emb
        
        emb = emb.repeat_interleave(repeats=num_frames, dim=0)
        if i2v_cond_time_zero:
            emb = rearrange(emb, "(b f) c -> b f c", f=num_frames)
            cond_emb = cond_emb.unsqueeze(1)
            emb = torch.cat([cond_emb, emb[:, 1:]], dim=1)
